...
<Tip>
The `sync-agent-symlinks` skill in `ai-specs/skills/sync-agent-symlinks/` automates the process of keeping `.claude/skills/` and `.cursor/skills/` aligned with `ai-specs/skills/` as the canonical source. Use it after any skill addition, removal, or rename to detect and repair broken or orphaned symlinks without manual inspection.

```bash
# The skill can be invoked directly by your agent:
# "Run the sync-agent-symlinks skill"
#
# It will inventory ai-specs/skills and both mirror targets,
# then add missing links, fix broken ones, and remove orphans.
```
</Tip>
