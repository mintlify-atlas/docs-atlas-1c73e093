...
description: "Install Specboot via npx or copy manually into your project. Node.js 18+ required. Existing files are never overwritten."
---
...
**Method 3 — Manual copy:** Use this when you need offline installation or want to inspect the files before they land in your project. The `-n` flag in `cp` prevents overwriting any files that already exist. After copying, create the four copilot symlinks by hand as shown above, then mirror `ai-specs/agents/` and `ai-specs/skills/` into `.claude/` and `.cursor/` using relative symlinks (see [Symlink Structure](#symlink-structure) below).
